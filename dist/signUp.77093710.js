  ...
  methods: {
    onSignUp: function onSignUp() {
      ...
      user.signUp().then(function (loggedInUser) {
        _this.$router.push({ name: '/', params: { message: 'signUpSuccess', email: _this.signUpData.email, password: _this.signUpData.password } });

        // this.$router.push({name:'/', params:{message:'signUpSuccess',userID:loggedInUser.id}})
      }, function (error) {
        if (error.code === 125) {
          _this.showPrompt('!', '请输入正确的邮箱地址！');
        } else if (error.code === 203) {
          _this.showPrompt('✘', '此电子邮箱已经被占用！');
        }
      });
    },
